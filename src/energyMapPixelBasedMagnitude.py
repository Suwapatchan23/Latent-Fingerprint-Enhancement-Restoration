...
def select_fingerprint_area(energy_img_list):
    ...
    for i in range(len(energy_img_list)):
        combined_img = energy_img_list[(i-1)%8] + energy_img_list[i] + energy_img_list[(i+1)%8]
...
output_path_3 = r"D:\KSIP_Research\Latent\Latent Fingerprint Enhancement & Restoration\output\magnitude_energy\watershed_magnitude_segment_fillhole/"
...
for idx in range(len(raw_files_path)):
    ...
    for i in range(len(path)):
        ...
        sector_fft = Fourier2D(gray_img)
        sector_fft.fft()
        sector_magnitude = sector_fft.getMagnitude()
    
        energy = find_energy(sector_magnitude, block_size=block_size)
        # plt.figure()
    ...
    new_segment_magnitude = select_fingerprint_area(energy_img_list)
    # plt.figure()
    ...
    # plt.figure()
    # plt.imshow(new_segment_magnitude, cmap="gray")
    # plt.figure()
    # plt.imshow(raw_magnitude, cmap="hot")
    # plt.figure()
    ...
    norm_image = cv.normalize(new_output_img, None, 0, 255, cv.NORM_MINMAX)
    uint8_image = norm_image.astype('uint8')
    # plt.figure()
    # plt.imshow(uint8_image, cmap="gray")

    # plt.show()

    # new_output_img = new_segment * raw_gray_img
    # new_output_img[new_segment == 0] = raw_gray_img.mean()
    ...
    # plt.show()
    # gray_output_img = cv.cvtColor(new_output_img, cv.COLOR_BGR2GRAY)
    ...
    cv.imwrite(output_file_name, uint8_image)
    plt.imsave(output_file_name, uint8_image)
    io.imsave(output_file_name, uint8_image)